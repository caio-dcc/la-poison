  ...
  try {
    ...
    for (const ing of ingredientsDict.ingredients) {
      ...
      if (res.ok) {
        ...
        const getRes = await fetch(`${REST_URL}/ingredients?name=eq.${encodeURIComponent(ing.name)}`, {
          headers,
        })
        if (getRes.ok) {
          const [existing] = await getRes.json()
          ingredientMap[ing.name.toLowerCase()] = existing.id
        }
      }
    }
    console.log(`✓ ${Object.keys(ingredientMap).length} ingredients ready\n`)

    // 3. Seed cocktails (batched)
    ...
    for (const batch of cocktailBatches) {
      ...
      if (res.ok) {
        ...
        process.stdout.write(`\r  ✓ ${cocktailCount}/${cocktails.length}`)
      } else {
    ...
    // 4. Seed cocktail_ingredients (batched)
